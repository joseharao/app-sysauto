...
uses UDados, UFuncoes, UCrec;

{$R *.dfm}

procedure TfrmLCrec.AbreCrec;
begin
   Application.CreateForm(TfrmCrec, frmCrec);
   frmCrec.sqlTab.Close;
   frmCrec.cdsTab.Close;
   frmCrec.sqlTab.ParamByName('COD_REC').AsInteger := cdsTabCOD_REC.AsInteger;
   frmCrec.Campo := 'COD_REC';
   frmCrec.cdsTab.Open;
   frmCrec.ShowModal;
...
procedure TfrmLCrec.btConsultarClick(Sender: TObject);
begin
MontaSql;
  inherited;
end;

procedure TfrmLCrec.btEditarClick(Sender: TObject);
begin
  inherited;
  AbreCrec;
...
procedure TfrmLCrec.grdDadosDblClick(Sender: TObject);
begin
  inherited;
  AbreCrec;
end;

procedure TfrmLCrec.grdDadosDrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
  inherited;
...
procedure TfrmLCrec.grdDadosKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
