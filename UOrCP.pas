...
uses UDados, URCrec, UFuncoes, Uerro, URCp;

{$R *.dfm}

procedure TfrmOrCp.btImprimirClick(Sender: TObject);
begin
  inherited;
 if application.MessageBox('Deseja Imprimir Relatórios de Contas a Receber Agora?', 'Confirmação', MB_YesNo + MB_IconQuestion) = idYes then begin
    try
       SQL := ' SELECT ' +
              ...
              ' CAST(COALESCE((SELECT SUM(CC.VLR_BAIXA) FROM CP_BX CC WHERE CC.COD_REC = C.COD_REC), 0) AS FLOAT) AS VLR_PAGO, ' +
       ...
       Application.CreateForm(TfrmRCp, frmRCp);
       with frmRCp do begin
           cdsTab.Close;
           sqlTab.Close;
           sqlTab.SQL.Clear;
           sqlTab.SQL.Add(SQL);
...
procedure TfrmOrCp.FormClose(Sender: TObject; var Action: TCloseAction);
begin
Dados.cdsComprador.Close;
  inherited;
end;

procedure TfrmOrCp.FormShow(Sender: TObject);
begin
  inherited;
