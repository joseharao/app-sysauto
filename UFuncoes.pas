...
interface
  ...
  Dialogs, Menus, ExtCtrls, StdCtrls, SqlExpr, IdHashMessageDigest, JPeg, DB, Math, RLReport;
  ...
  procedure AbreFormMDI(Componete :  TComponentClass; Nome : TForm);
  Procedure GravaFoto( NomeCampo: TBlobField; FileName: String );
  Procedure ObtemFoto( NomeCampo: TBlobField; Var imgDestino: TImage );
  Procedure ObtemFotoRel( NomeCampo: TBlobField; Var imgDestino: TRlImage );
...
begin;
  ...
  for x := 1 to Length(str) do begin
     if (copy(Str, x, 1) <> '&') then begin
        str2 := str2 + Copy(Str, x, 1);
     end;
  end;
  Result := Str2;
...
//procedimento para abre formulario em uma linha so mdi
procedure AbreFormMDI(Componete :  TComponentClass; Nome : TForm);
begin
